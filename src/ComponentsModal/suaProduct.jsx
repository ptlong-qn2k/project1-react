...
const ModalEdit = ({ setOpen2, open2, idUpdate }) => {
    ...
    const handleClose2 = (data) => {
        setDataEdit({ title: data.title, sku: data.sku, weight: data.weight, price: data.price })
    ...
    return (
        ...
        >
            <Box sx={style} className="w-full h-full bg-white ">
                ...
                <form onSubmit={handleSubmit(handleClose2)} className='w-[377px] h-[26px] flex flex-col ml-10 mb-[22px] mt-7 justify-between items-start'>
                    <label className='flex w-[300px] flex-row justify-between items-center'>title:
                        <input placeholder='  title' value={dataEdit.title} className="bg-[#F1F1F1] w-[245px] h-[14px] text-xs "
                            {...register("title",)}
                        />
                    </label>
                    ...
                    <label className='flex w-[300px] flex-row justify-between items-center'>sku :
                        <input placeholder='  sku' value={dataEdit.sku} {...register("sku")} className="bg-[#F1F1F1] w-[245px] h-[14px] text-xs" />
                    </label>
                    {errors?.sku?.type === "pattern" && (
                        <p>Alphabetical characters only</p>
                    )}
                    <label className='flex w-[300px] flex-row justify-between items-center'>weight:
                        <input placeholder='  weight' value={dataEdit.weight} {...register("weight",)} className="bg-[#F1F1F1] w-[245px] h-[14px] text-xs " />
                    </label>
                    {errors.weight && (
                    ...
                    <label className='flex w-[300px] flex-row justify-between items-center'>price:
                        <input placeholder='  price' value={dataEdit.price} {...register("price",)} className="bg-[#F1F1F1] w-[245px] h-[14px] text-xs " />
                    </label>
                    {errors.price && (
                        <p>you must be older then o and price 99 $</p>
                    )}
                    <input type="submit" value="edit" className=' cursor-pointer rounded-[10px] w-[85px] h-[26px] bg-[#004744] text-white flex flex-row items-center justify-center' />
